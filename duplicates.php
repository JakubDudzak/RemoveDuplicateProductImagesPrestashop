...
// === Safe delete via PrestaShop API ===
function safeDeleteImage($id_image)
{
...
if (isset($_GET['delete_id'])) {
    ...
    if (safeDeleteImage($delete_id)) {
        echo '<p style="color:green;">Image ID ' . $delete_id . ' was deleted safely.</p>';
    } else {
        echo '<p style="color:red;">Failed to delete image ID ' . $delete_id . ' (not found or error).</p>';
    }
}
...
if (isset($_GET['delete_all']) && $_GET['delete_all'] == 1) {
    if (!empty($_POST['all_ids'])) {
        ...
        echo '<p style="color:green;">Successfully deleted ' . $deleted_count . ' duplicate images via PrestaShop API.</p>';
    }
}
...
echo '<table border="1" cellpadding="4" style="border-collapse: collapse; width:100%; font-family:Arial, sans-serif;">
    ...
    foreach ($images as $img) {
        ...
        if ($duplicate_of !== null) {
            ...
            echo '<tr>
                <td>' . $id_product . '</td>
                <td>' . $id_image . '</td>
                <td>ID ' . $duplicate_of . ($mode === 'visual' ? " (visual)" : " (exact)") . '</td>
                <td><a href="' . htmlspecialchars($frontLink) . '" target="_blank">?? View Product</a></td>
                <td><a href="?delete_id=' . $id_image . '&mode=' . $mode . '" onclick="return confirm(\'Delete this duplicate?\');">Delete</a></td>
            </tr>';
...
if ($dup_count > 0) {
    echo '<form method="POST" action="?delete_all=1&mode=' . $mode . '" onsubmit="return confirm(\'Are you sure you want to delete all ' . $dup_count . ' duplicate images?\');">';
    echo '<input type="hidden" name="all_ids" value="' . implode(',', $duplicate_ids) . '">';
    echo '<button type="submit" style="margin:10px 0; background-color:#c00; color:white; padding:6px 12px; border:none; border-radius:4px; cursor:pointer;">Delete All (' . $dup_count . ')</button>';
    echo '</form>';
} else {
...
if ($hash_failures > 0) {
    echo '<p style="color:orange;">Skipped ' . $hash_failures . ' images (could not generate hash).</p>';
}
